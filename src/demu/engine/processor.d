...
class CPU {
    ...
    void doOp() {
        ubyte data = ram.read(state.programCounter);
        final switch (data) {
            // default: assert(0, "Impossible Opcode");
    ...
    ushort getU16() {
        return Data2(
            ram.read(state.programCounter+1),
            ram.read(state.programCounter+2)
        ).u16;
    }

    // ubyte getU8() {
    //     return ram.read(state.programCounter+1);
    // }
    
    ubyte getU8(ushort off) {
        return ram.read(state.programCounter+off);
    }
    ...
    ubyte* fromRef(ushort address) {
        return ram.read(address);
    }


    void opLoad(ubyte* dest, ubyte source) {
        ram.write(dest, source);
    }


    void opJump(ushort address) {
    ...
    void opIncrement(T)(T* var) {
        *var += 1;
    }
    
    void opDecrement(T)(T* var) {
        *var -= 1;
    }

    void opShiftLeft(char mode)(ubyte* var, ) {
        // ubyte inVal = 0;
        // ubyte outVal = *var << 7;
        // static if ('c') inVal = state.fC << 7;
        // static if ('7') inVal = *var & 0x80;
        // static if ('0') inVal = outVal;
        // *var = (*var >> 1) | inVal;
        // state.fC = outVal;
    }

    void opShiftRight(char mode)(ubyte* var) {
        // ubyte inVal = 0;
        // ubyte outVal = *var >> 7;
        // static if ('c') inVal = state.fC;
        // static if ('7') inVal = outVal;
        // static if ('0') inVal = *var & 0x01;
        // *var = (*var << 1) | inVal;
        // state.fC = outVal;
    }
}
